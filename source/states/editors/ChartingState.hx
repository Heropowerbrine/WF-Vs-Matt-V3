	...
	**/
	...
	override function create()
	{
		...
		if (_song.keyCount != null)
			PlayState.keyCount = _song.keyCount;
		...
		nightmareMatt = new FlxSprite(0, -1800);
		nightmareMatt.frames = Paths.getSparrowAtlas('editors/charter/Jumpscare');
		nightmareMatt.animation.addByPrefix('Jumpscare', 'Jumpscare', 24, false);
		nightmareMatt.screenCenter(X);
		nightmareMatt.scrollFactor.set();
	...
	function addSongUI():Void
	{
		...
		var stepperKeyCount:FlxUINumericStepper = new FlxUINumericStepper(100, stepperSpeed.y, 1, 4, 1, 9, 0);
		if (_song.keyCount != null)
	...
	function addSectionUI():Void
	{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
				note[1] = note[1] < PlayState.keyCount ? (note[1] + PlayState.keyCount) : (note[1] - PlayState.keyCount);
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			{
				var boob = note[1];
				if (boob>PlayState.keyCount-1){
					boob -= PlayState.keyCount;
				}else{
					boob += PlayState.keyCount;
				}
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				var boob = note[1]%PlayState.keyCount;
				boob = (PlayState.keyCount-1) - boob;
				if (note[1] > PlayState.keyCount-1) boob += PlayState.keyCount;
	...
	override function update(elapsed:Float)
	{
		...
		camPos.x = (strumLine.x + CAM_OFFSET) + (GRID_SIZE * (columns-9) * 0.75);
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(curSelectedNote != null) {
				...
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += PlayState.keyCount;
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					var data:Int = note.noteData % PlayState.keyCount;
					var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += PlayState.keyCount;
					...
					if(!playedSound[data]) {
						if(note.hitsoundChartEditor && ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)))
						{
							...
							FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < PlayState.keyCount ? -0.3 : 0.3; //would be coolio
		...
		if (FlxG.mouse.justPressed && !HealthNSafety.unlock)
		{
			if (PlayState.SONG.song.toLowerCase() == 'foulplay' && nightmareMatt.alpha < 0.1) //change this to foulplay later
			{
				nightmareMatt.alpha = 1;
				nightmareMatt.animation.play('Jumpscare');
				FlxG.sound.play(Paths.sound('jumpscare')); //should we use the indie cross sound? 		its literally just a base game sound lol - zoro
				FlxG.sound.play(Paths.sound('nightmareeffect'));
				
				FlxG.sound.music.pause();
				vocals.pause();

				new FlxTimer().start(nightmareMatt.animation.curAnim.frames.length/24, function(tmr) {
					FlxG.camera.alpha = 0;
	...
	function reloadGridLayer() {
		columns = (PlayState.keyCount*2)+1;
		GRID_SIZE = Std.int((40+9)-columns);
		if (dummyArrow != null)
		{
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * PlayState.keyCount)).makeGraphic(1, 1, FlxColor.BLACK);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[1] > PlayState.keyCount-1) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo % PlayState.keyCount, null, null, true);
		...
		if(isNextSection && _song.notes[curSec].mustHitSection != _song.notes[curSec+1].mustHitSection) {
			if(daNoteInfo > PlayState.keyCount-1) {
				note.x -= GRID_SIZE * PlayState.keyCount;
			} else if(daSus != null) {
				note.x += GRID_SIZE * PlayState.keyCount;
			}
		}
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += PlayState.keyCount;
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	public function doANoteThing(cs, d, style){
		...
		if(strumLineNotes.members[d].overlaps(curRenderedNotes))
		{
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.overlapsPoint(new FlxPoint(strumLineNotes.members[d].x + 1,strumLine.y+1)) && note.noteData == d % PlayState.keyCount)
				{
						//trace('tryin to delete note...');
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + PlayState.keyCount) % PlayState.keyCount*2, noteSus, curNoteTypes[daType]]);
		}
