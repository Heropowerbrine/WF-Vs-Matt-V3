...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			var sky = new FlxSprite(-450, -175).loadGraphic(Paths.image('arena_/skyBG'));
			sky.scrollFactor.set(0.3, 0.3);
			sky.scale.set(0.9, 0.9);
			sky.updateHitbox();
			add(sky);

			var stands = new FlxSprite(-450, -225).loadGraphic(Paths.image('arena_/standsBG'));
			stands.scrollFactor.set(0.47, 0.47);
			stands.scale.set(0.9, 0.9);
			stands.updateHitbox();
			add(stands);

			var railingnew = new FlxSprite(-450, -200).loadGraphic(Paths.image('arena_/railingBG'));
			railingnew.scrollFactor.set(0.52, 0.52);
			railingnew.scale.set(0.9, 0.9);
			railingnew.updateHitbox();
			add(railingnew);

			var groundBG = new FlxSprite(-450, -100).loadGraphic(Paths.image('arena_/groundBG'));
		...
		// Characters
		gf = new Character(350, 200, 'sportsgf');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		boyfriend = new Character(850, 200, 'sportsbf', true);
		...
		{
			var numScore:FlxSprite = new FlxSprite(36 * daLoop).loadGraphic(Paths.image('num' + i));
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.77 : 0), 'ui/healthBar', null, 0, 2);
	...
	function repositionCombo()
	{
		...
		rating.y -= 0 + ClientPrefs.data.comboOffset[1];

		comboNums.screenCenter();
		comboNums.y = healthBar.y + (ClientPrefs.data.downScroll ? 160 : -25);
		//comboNums.x -= ((comboNums.length/2)-0.5)*42;
		comboNums.x += ClientPrefs.data.comboOffset[2];
		comboNums.y += 0 - ClientPrefs.data.comboOffset[3];
		reloadTexts();
	}
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch(i)
			{
				...
				case 4:
					var resetBindsText:String = "";
					var bind1:Null<FlxKey> = ClientPrefs.keyBinds.get("reset")[0];
					var bind2:Null<FlxKey> = ClientPrefs.keyBinds.get("reset")[1];
					if (bind1 != null && bind1 != NONE) {
						resetBindsText += '"' + InputFormatter.getKeyName(bind1) + '"';
					}
					if (bind2 != null && bind2 != NONE) {
						if (resetBindsText != "") resetBindsText += " or ";
						resetBindsText += '"' + InputFormatter.getKeyName(bind2) + '"';
					}
					
					dumbTexts.members[i].text = 'Press '+resetBindsText+' to reset'; 
			}
		}
	}
